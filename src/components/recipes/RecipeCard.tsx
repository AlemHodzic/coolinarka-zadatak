...
import { Recipe, difficultyLabels } from '@/types/recipe'
...
export function RecipeCard({ recipe }: RecipeCardProps) {
  ...
  return (
    <Link 
      ...
      aria-label={`${recipe.title} - ${difficultyLabels[recipe.difficulty]}, ${recipe.prepTime} minuta`}
    >
      <article className="recipe-card">
        ...
        <div className="p-5">
          ...
          <div className="flex items-center gap-4 text-sm text-warm-500">
            <span className="flex items-center gap-1" aria-label={`Vrijeme pripreme: ${recipe.prepTime} minuta`}>
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              <span>{recipe.prepTime} min</span>
            </span>
            <span className="flex items-center gap-1" aria-label={`Broj porcija: ${recipe.servings}`}>
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
